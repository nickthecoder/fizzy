...
class SnapPoint(val point: Dimension2Expression)
    : HasChangeListeners<SnapPoint>, PropListener, MetaDataAware {
    ...
    override val changeListeners = ChangeListeners<SnapPoint>()
    ...
    /**
     * Any changes to the SnapPoint's data causes this [Prop]'s propListeners to be notified.
     */
    ...
    fun index(): Int {
        shape?.let { shape ->
            shape.snapPoints.forEachIndexed { index, cp ->
                if (cp === this) {
                    return index
                }
            }
    ...
    fun copy(link: Boolean) = SnapPoint(point.copy(link))
